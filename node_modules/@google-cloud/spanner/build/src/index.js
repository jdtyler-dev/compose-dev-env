...
class Spanner extends service_1.GrpcService {
    ...
    static getSpannerEmulatorHost() {
        ...
        if (endpointWithPort) {
            if (endpointWithPort.startsWith('http:') ||
                ...
                throw new Error('SPANNER_EMULATOR_HOST must not start with a protocol specification (http/https)');
            }
    ...
    createInstance(name, config, callback) {
        ...
        if (!config) {
            throw new Error(['A configuration object is required to create an instance.'].join(''));
        }
        ...
        if (reqOpts.instance.nodeCount && reqOpts.instance.processingUnits) {
            throw new Error(['Only one of nodeCount or processingUnits can be specified.'].join(''));
        }
        if (!reqOpts.instance.nodeCount && !reqOpts.instance.processingUnits) {
    ...
    prepareGapicRequest_(config, callback) {
        this.auth.getProjectId((err, projectId) => {
            ...
            if (!this.projectIdReplaced_) {
                this.projectId = projectify_1.replaceProjectIdToken(this.projectId, projectId);
                this.projectFormattedName_ = projectify_1.replaceProjectIdToken(this.projectFormattedName_, projectId);
                this.instances_.forEach(instance => {
                    instance.formattedName_ = projectify_1.replaceProjectIdToken(instance.formattedName_, projectId);
                    instance.databases_.forEach(database => {
                        database.formattedName_ = projectify_1.replaceProjectIdToken(database.formattedName_, projectId);
                    });
                });
                this.projectIdReplaced_ = true;
            }
            config.headers[common_1.CLOUD_RESOURCE_HEADER] = projectify_1.replaceProjectIdToken(config.headers[common_1.CLOUD_RESOURCE_HEADER], projectId);
